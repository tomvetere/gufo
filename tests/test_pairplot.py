...
class TestPairplotConstruction:
    ...
    def test_column_subset(self, numeric_df):
        g = cerno.pairplot(numeric_df, columns=["a", "b"])
        fig, axs = g._render()
...
class TestPairplotRendering:
    def test_saves(self, numeric_df, tmp_path):
        ...
        cerno.pairplot(numeric_df).save(path)
        assert path.exists()
    ...
    def test_with_color(self, iris_df, tmp_path):
        ...
        cerno.pairplot(iris_df, color="species").save(path)
        assert path.exists()
    ...
    def test_title(self, numeric_df):
        g = cerno.pairplot(numeric_df).title("Pair Plot")
        fig, axs = g._render()
    ...
    def test_custom_figsize(self, numeric_df):
        g = cerno.pairplot(numeric_df, figsize=(12, 12))
        fig, axs = g._render()
...
class TestPairplotDataFormats:
    ...
    def test_polars_data(self, tmp_path):
        ...
        cerno.pairplot(df).save(path)
        assert path.exists()

    def test_color_column_excluded_from_grid(self, iris_df):
        g = cerno.pairplot(iris_df, color="species")
        fig, axs = g._render()
